          ...
          <v-card-text>
            <v-container>
              <v-row>
                ...
                <v-col cols="12" sm="6">
                  <v-text-field v-model="editedItem.quantidadeCamas" label="Quantidade de Camas" type="number" required></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                ...
                <v-col cols="12" sm="6">
                  <v-select v-model="editedItem.tipoBanheiro" :items="tipoBanheiroOptions" label="Tipo de Banheiro" required></v-select>
                </v-col>
                <v-col cols="12" sm="6">
                ...
                <v-col cols="12" sm="6" v-if="editedItem.comTV">
                  <v-text-field v-model="editedItem.tamanhoTV" label="Tamanho da TV (polegadas)" type="number"></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" v-if="editedItem.comTV">
                ...
                <v-col cols="12" sm="6">
                  <v-text-field v-model="editedItem.preco" label="Preço" prefix="R$" type="number" required></v-text-field>
                </v-col>
  ...
  export default {
    ...
    methods: {
      async fetchAcomodacoes() {
        try {
          const response = await axios.get(`http://localhost:3000/acomodacao/`);
      ...
      deleteItem(item) {
        ...
        confirm('Tem certeza que deseja excluir esta acomodação?') &&
          axios
            .delete(`http://localhost:3000/acomodacao/${item.id}`)
            .then(() => {
      ...
      save() {
        if (this.editedIndex > -1) {
          // Editar acomodação
          axios
            .put(`http://localhost:3000/acomodacao/${this.editedItem.id}`, this.editedItem)
            .then(() => {
          ...
          axios
            .post(`http://localhost:3000/acomodacao/`, this.editedItem)
            .then((response) => {
