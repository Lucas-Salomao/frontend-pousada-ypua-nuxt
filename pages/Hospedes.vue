<template>
  ...
  export default {
    ...
    methods: {
      async fetchHospedes() {
        try {
          const response = await axios.get(`http://localhost:3000/hospede/`);
      ...
      deleteItem(item) {
        ...
        confirm('Tem certeza que deseja excluir este hóspede?') &&
          axios
            .delete(`http://localhost:3000/hospede/${item.id}`) // Substitua pelo endpoint da sua API
            .then(() => {
      ...
      save() {
        if (this.editedIndex > -1) {
          axios
            .put(`http://localhost:3000/hospede/${this.editedItem.id}`, this.editedItem) // Substitua pelo endpoint da sua API
            .then(() => {
          ...
          axios
            .post(`http://localhost:3000/hospede/`, this.editedItem) // Substitua pelo endpoint da sua API
            .then((response) => {
